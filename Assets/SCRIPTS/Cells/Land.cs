...
public class Land : MonoBehaviour {
    ...
    bool closed = true; // Закрыта ли клетка
    string[] CanGo; // С этих клеток можно попасть сюда
    Transform pirate; // Transform пирата, который уже стоит на клетке
    Vector3 TempVector; // Вспомогательная переменная для переноса клеток при землетрясении
    string TempName; // Вспомогательная переменная для переноса клеток при землетрясении
    Sprite OpenSprite; // Sprite открытой клетки
    int[] CellNum; // Номер клетки
    // Словарь с ключами - номерами позиций и значениями - координатами, куда встает пират на этой позиции
    Dictionary<string, Vector3> StayPoints = new Dictionary<string, Vector3>(9);
    Dictionary<string, int> drops = new Dictionary<string, int>(2)    {
    {"big", 0},
    {"gold", 0},
    };
    ...
    void OnMouseDown() {
        ...
        if (Map.EarthQuake == false && Map.LightHouse == false && (Map.child.childCount > 0 && closed) == false && (Map.plane || Map.PirateCanGo[Map.child.name].IndexOf(name) != -1) && Map.select == 1) {
            ...
            foreach (string i in Map.PirateOnCells.Keys) {
                ...
                if (Map.PirateOnCells[i][0] == name) {
                    ...
                    switch (i) {
                        ...
                        default:
                            for (int ii = 0; ii != ChildCount; ii++) { // Возврат всего дропа обратно ибо нападать с дропом нельзя
            ...
            for (int i = 0; i != ChildCount; i++) {
                ...
                drops[Map.child.GetChild(i).name]++;
            }
        ...
        if (Map.LightHouse && closed && Map.EarthQuake == false) {
            ...
            if (LightHouse.OpenCount > 4){ Map.LightHouse = false; }
        }
        // Землетрясение
