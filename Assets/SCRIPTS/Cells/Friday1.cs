...
public class Friday1 : MonoBehaviour {
    ...
    void OnMouseDown() {
        ...
        if ((closed && Map.child.childCount > 0) == false && (Map.plane || Map.PirateCanGo[Map.child.name].IndexOf(name) != -1) && Map.select == 1) {
            ...
            if (closed) {
                ...
                // Пятница переностится на эту клетку
                fri.localPosition = StayPoints[Map.PirateOnCells[fri.name][1]];
                // Записывается где теперь Пятница и куда он может пойти
                Map.PirateOnCells[fri.name][0] = name;
                Map.PirateCanGo[fri.name] = string.Join("+", CanGo);
                // Настройка Пятницы
